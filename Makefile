...
tag:
	...
	"$(shell LANG=en_US.UTF-8 svn info | grep ^Root: | sed -e 's/^[^:]*: //')/tags/$(SVNTAG)" \
	-m "tagged release $(VERSION)"

archive:
	TMPSUBDIR=`mktemp -d`
	cd "${TMPSUBDIR}"; svn export --force $(shell LANG=en_US.UTF-8 svn info | grep ^Root: | sed -e 's/^[^:]*: //')/tags/$(SVNTAG) "${TMPSUBDIR}"/${NAME}-${VERSION}
...
snapshot:
	TMPSUBDIR=`mktemp -d`
	cd "${TMPSUBDIR}"; svn export --force $(shell LANG=en_US.UTF-8 svn info | grep ^Root: | sed -e 's/^[^:]*: //')/trunk/$(NAME) "${TMPSUBDIR}"/${NAME}-${VERSION}
	rm -f "${TMPSUBDIR}"/${NAME}-${VERSION}/Makefile
